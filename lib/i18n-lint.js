...
var error = function (code, reason, evidence, character, scope, line) {
	...
	return utils.getLast(I18NLINT.errors);
};
...
var reportErrors = function () {
	var currentTag = utils.getLast(I18NLINT.tagStack) || {};
...
I18nLint.scan = function (lines, options) {
	...
	options = extend({
		attributes: ['alt', 'placeholder', 'title', 'label', 'value'],
	...
	var parser = new htmlparser.Parser({
		...
		ontext: function (text) {
			...
			var isHardcoded = utils.isHardcodedString(text);
			if (
		...
		onattribute: function (name, value) {
			...
			var isHardcoded = utils.isHardcodedString(value);
			if (
